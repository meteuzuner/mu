...
def elliptical_annulus_photometry(
    ...
    """
    Perform elliptical annulus photometry on a 2D map. The annulus is defined by
    two ellipses (inner and outer), both sharing the same center (RA, Dec) and position angle.

    The function calculates the weighted mean of the pixel values in the annulus
    (accounting for partial pixels), then returns that mean and its error.

    Parameters
    ----------
    data : np.ndarray
        2D array of flux or intensity values.
    header : fits.Header
        FITS header corresponding to `data` (for WCS).
    center_ra : Quantity
        Right Ascension of the annulus center. If unitless, assume degrees.
    center_dec : Quantity
        Declination of the annulus center. If unitless, assume degrees.
    inner_semi_major : Quantity
        Inner ellipse's semi-major axis. If unitless, assume degrees.
    inner_semi_minor : Quantity
        Inner ellipse's semi-minor axis. If unitless, assume degrees.
    outer_semi_major : Quantity
        Outer ellipse's semi-major axis. If unitless, assume degrees.
    outer_semi_minor : Quantity
        Outer ellipse's semi-minor axis. If unitless, assume degrees.
    position_angle : Quantity
        Position angle for both ellipses. If unitless, assume degrees.
    method : {'mean'}, optional
        Which statistic to compute. Currently, only "mean" is implemented.
    quick_plot : bool, optional
        If True, display a quick plot showing the data and both ellipses.

    Returns
    -------
    mean_val : float
        Weighted mean of the pixel values in the elliptical annulus.
    mean_err : float
        Approximate error on the mean: std / sqrt(N_eff).
    """
    ...
    if not isinstance(header, fits.Header):
        raise TypeError(f"'header' must be a fits.Header, got {type(header)}.")
        
    if method.lower() != "mean":
        raise NotImplementedError(f"Only 'mean' is implemented. You requested {method!r}.")
      
    # --- 2) Ensure angles have correct units (assume deg if unitless) --- 
    def _ensure_deg(quantity, name="angle"):
        """
        Ensures the given quantity is in degrees. If unitless, assumes degrees.
        """
        if isinstance(quantity, (int, float)):  # Handle unitless numbers
            print(f"Parameter '{name}' has no unit. Assuming degrees.")
            return quantity * u.deg
        elif hasattr(quantity, 'unit'):  # If it has a unit, convert to degrees
            try:
                return quantity.to(u.deg)
            except u.UnitConversionError:
                raise ValueError(f"Parameter '{name}' has incompatible units: {quantity.unit}")
        else:
            raise TypeError(f"Parameter '{name}' must be a number or an astropy Quantity.")

    center_ra  = _ensure_deg(center_ra,  "center_ra")
    center_dec = _ensure_deg(center_dec, "center_dec")
    inner_semi_major = _ensure_deg(inner_semi_major, "inner_semi_major")
    inner_semi_minor = _ensure_deg(inner_semi_minor, "inner_semi_minor")
    outer_semi_major = _ensure_deg(outer_semi_major, "outer_semi_major")
    outer_semi_minor = _ensure_deg(outer_semi_minor, "outer_semi_minor")
    position_angle = _ensure_deg(position_angle, "position_angle")

    # --- 3) Build WCS and the two sky ellipses (inner, outer) ---
    wcs = WCS(header)
    center_skycoord = SkyCoord(ra=center_ra, dec=center_dec, frame='icrs')

    # Outer ellipse
    aperture_outer_sky = SkyEllipticalAperture(
        positions=center_skycoord,
        a=outer_semi_major,
        b=outer_semi_minor,
        theta=position_angle)
    # Inner ellipse
    aperture_inner_sky = SkyEllipticalAperture(
        positions=center_skycoord,
        a=inner_semi_major,
        b=inner_semi_minor,
        theta=position_angle)
    ...
    aperture_outer_pix = aperture_outer_sky.to_pixel(wcs)
    aperture_inner_pix = aperture_inner_sky.to_pixel(wcs)

    # --- 4) Construct partial-pixel masks with method='exact' ---
    mask_outer = aperture_outer_pix.to_mask(method='exact')
    mask_inner = aperture_inner_pix.to_mask(method='exact')
    
    # coverage fraction for each pixel
    cov_outer = mask_outer.data
    cov_inner = mask_inner.data

    # Because these two masks might have different bounding boxes,
    # we need to place them onto the same coordinate grid in order to do subtraction.
    # The bounding boxes can differ if the inner ellipse is strictly inside the outer ellipse.
    # The easiest is to use .to_image(data.shape) or combine them carefully.

    # Photutils has a convenience method .to_image(shape) -> 2D array with coverage,
    # placed in the correct position within 'shape'.
    # We'll do that for both outer & inner, then subtract.

    coverage_outer_full = mask_outer.to_image(data.shape)
    coverage_inner_full = mask_inner.to_image(data.shape)
    
    coverage_annulus = coverage_outer_full - coverage_inner_full
    coverage_annulus = np.clip(coverage_annulus, 0, 1)

    # coverage_annulus[i] in [0..1], data[i] can be positive or negative

    # Instead of storing partial flux = data * coverage,
    # gather coverage and the original data in arrays for valid pixels:
    valid = (coverage_annulus > 0) & np.isfinite(data)

    coverage_vals = coverage_annulus[valid]
    data_vals = data[valid]

    # Weighted mean in data space:
    # mean = sum(w_i * x_i) / sum(w_i), with w_i=coverage_i, x_i=data_i
    w_sum = np.sum(coverage_vals)
    mean_val = np.sum(coverage_vals * data_vals) / w_sum

    # Weighted variance in data space:
    # var = sum(w_i * (x_i - mean)^2) / sum(w_i)
    var_val = np.sum(coverage_vals * (data_vals - mean_val)**2) / w_sum
    std_val = np.sqrt(var_val)

    # The "effective number of pixels"
    n_eff = w_sum

    # Error on mean => std / sqrt(n_eff)
    mean_err = std_val / np.sqrt(n_eff)

    print(f"Mean in annulus = {mean_val:.4e} +/- {mean_err:.4e}")
    print(f"Std  in annulus = {std_val:.4g}\n")

    # --- 5) Optional quick_plot to visualize data & annulus ---
    if quick_plot:
        ...
        # Overplot both ellipses
        aperture_outer_pix.plot(ax=ax, color='red', lw=1.5)
        aperture_inner_pix.plot(ax=ax, color='red', lw=1.5, ls='--')
        ax.set_title("Elliptical Annulus Photometry")
        
        plt.show()

    return mean_val, mean_err, std_val
