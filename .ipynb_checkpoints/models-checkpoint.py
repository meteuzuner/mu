...
def modified_blackbody(
    ...
    F_nu_si = B_nu_si * factor # W m^-2 Hz^-1 sr^-1
    
    F_nu_jy_sr  = F_nu_si * 1e26 * (u.Jy / u.sr)
    F_nu_jy_as2 = F_nu_jy_sr.value / (360 * 3600 / 2*np.pi)**2 * (u.Jy / u.arcsec**2) # 1 radian = 360 / 2*pi = 57.296 degrees
    
    # -------------------------------------
    # Optionally quick-plot (log-log scale)
    # -------------------------------------
    if quick_plot:
        freq_vals = nu_si  # in Hz
        wavelength_vals = c/nu_si * 1e6 # in µm
        flux_vals = F_nu_jy_as2
        plt.figure(figsize=(7, 5))
        plt.loglog(wavelength_vals, F_nu_jy_as2, label=fr"M={Md_si:.1e}, T={T:.1f}, $\beta$={beta:.1f}", color="red", lw=2)
